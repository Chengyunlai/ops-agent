...
class CapabilityRegistration(_AgentModel):
    """专业 Agent 提交给全局 Policy 的可信能力定义。"""

    capability: CapabilityId
    resources: frozenset[ResourceKind] = Field(default_factory=frozenset)
    tool_names: frozenset[NonBlankText] = Field(default_factory=frozenset)
...
class CapabilityRegistry(_AgentModel):
    ...
    def capability_for_resource(self, resource: ResourceKind) -> CapabilityId | None:
        for registration in self.registrations:
            if resource in registration.resources:
                return registration.capability
        return None

    def tool_names_for(self, capability: CapabilityId) -> frozenset[str]:
        for registration in self.registrations:
            if registration.capability is capability:
                return frozenset(registration.tool_names)
        return frozenset()
