...
from ops_agent.agent.orchestration.interpretation import IntentInterpreter
from ops_agent.agent.orchestration.policy import (
...
def create_ops_agent(
    ...
    capabilities = create_kubernetes_capability_registry(
        tool.name for tool in kubernetes_tools
    )
    direct_agents = {
        capability: KubernetesAgent(
            ...
            [
                ...
                for tool_name in sorted(capabilities.tool_names_for(capability))
            ],
        )
        for capability in capabilities.enabled
    }
    plan_tool_names = capabilities.tool_names_for(
