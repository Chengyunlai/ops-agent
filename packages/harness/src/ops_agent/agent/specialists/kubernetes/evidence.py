...
_DIAGNOSTICS_TOOL = "diagnose_kubernetes_workloads"
_EVENTS_TOOL = "list_kubernetes_events"
_LOGS_TOOL = "get_kubernetes_pod_logs"
...
class KubernetesEvidenceCollector:
    ...
    def collect_workload_health(self) -> KubernetesEvidence:
        result, issue = self._invoke(_DIAGNOSTICS_TOOL, {})
    ...
    def collect_supporting_evidence(
        ...
        for pod_name in pod_names:
            self._collect(
                tool_name=_EVENTS_TOOL,
                arguments={"pod_name": pod_name, "limit": 100},
                source=f"{_EVENTS_TOOL}:Pod/{pod_name}",
        ...
        for pod_name, container_name in previous_log_targets:
            self._collect(
                ...
                source=f"{_LOGS_TOOL}:Pod/{pod_name}/{container_name}:previous",
                observations=observations,
                issues=issues,
            )
        return KubernetesEvidence(
